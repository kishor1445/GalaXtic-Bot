...
class Anime(commands.Cog):
    ...
    async def search_anilist(self, query):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json={"query": query_str, "variables": variables}) as resp:
    ...
    async def add_anime(self, interaction: discord.Interaction, name: str):
        ...
        if len(results) == 1:
            ...
            msg = await interaction.followup.send(
                content="Select the correct anime:",
                view=view
            )
            view.message = msg
    ...
    async def send_anime_confirmation(self, interaction, anime, message=None):
        ...
        title = (anime['title']['english'] or anime['title']['romaji']) + f" ({anime_type})"
        ...
        embed.set_footer(text="This confirmation will expire in 5 minutes." if not already_added else "This anime is already in your list.")
        status = anime.get('status', '').upper()
        episodes = anime.get('episodes')
        next_ep = anime.get('nextAiringEpisode')
        if status == "RELEASING":
            last_ep = str(next_ep['episode'] - 1) if next_ep and next_ep.get('episode') else "?"
            next_ep_num = str(next_ep['episode']) if next_ep and next_ep.get('episode') else "?"
            next_ep_time = f"<t:{next_ep['airingAt']}:R>" if next_ep and next_ep.get('airingAt') else "?"
            episodes_display = f"Ongoing\nLast: {last_ep}\nNext: {next_ep_num} ({next_ep_time})"
        ...
        embed.add_field(name="Season", value=f"{anime.get('season', 'N/A')} {anime.get('seasonYear', 'N/A')}", inline=True)
        genres = ', '.join(anime.get('genres', [])) or 'N/A'
        embed.add_field(name="Genres", value=genres, inline=False)
        if anime.get('id', None) is not None:
            embed.add_field(name="Streaming", value=f"[Watch Free](https://www.miruro.tv/watch?id={anime.get('id')})")

        if already_added:
            if message:
                await message.edit(content="This anime is already in your list!", embed=embed, view=None)
            else:
                await interaction.followup.send(content="This anime is already in your list!", embed=embed, view=None)
            return
    ...
    @app_commands.command(name="remove_anime", description="Remove an anime/movie from your list")
    ...
    async def remove_anime(self, interaction: discord.Interaction, name: str):
        ...
        await interaction.response.send_message(f"Removed '{name}' from your list (if it existed).", ephemeral=True)
...
class AnimeConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if check_result and check_result[0].get('result'):
            await interaction.response.send_message("This anime is already in your list!", ephemeral=True)
            return
        await db.query(
        ...
        await interaction.response.edit_message(content="Anime added to your list!", view=None)
...
class AnimeSelect(discord.ui.Select):
    def __init__(self, results, user_id, anime_cog):
        ...
        super().__init__(placeholder="Choose the correct anime...", min_values=1, max_values=1, options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if anime:
            ...
            await interaction.response.edit_message(content="Loading anime details...", view=None)
            await self.anime_cog.send_anime_confirmation(interaction, anime, message=interaction.message)
        else:
            await interaction.response.send_message("Anime not found in selection.", ephemeral=True)
