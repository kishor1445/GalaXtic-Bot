...
class Fun(commands.Cog):
    ...
    @app_commands.command(name="set_count_channel", description="Set a channel for counting numbers (admin only)")
    ...
    async def set_count_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        await db.query(
            f"UPDATE count_channel SET channel_id='{channel_id}' WHERE guild_id='{guild_id}';"
            f"IF $result = NONE THEN CREATE count_channel SET guild_id='{guild_id}', channel_id='{channel_id}'; END;"
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        result = await db.query(f"SELECT * FROM count_channel WHERE guild_id='{guild_id}' LIMIT 1")
        if not result or not result[0]:
            return
        count_info = result[0]
        if count_info.get('channel_id') != channel_id:
            return
        ...
        def safe_eval(expr):
            ...
            try:
                ...
                for n in ast.walk(node):
                    if not isinstance(n, (ast.Expression, ast.BinOp, ast.UnaryOp, ast.Constant, ast.operator, ast.unaryop, ast.Load)):
                        return None
        ...
        if number != current_count + 1:
            await message.reply(f"❌ Wrong number {message.author.display_name}! The next number should be {current_count + 1}.\n\n Counting has been reset.")
    ...
    @app_commands.command(name="random_choice", description="Pick a random item from a comma-separated list")
    ...
    async def verify_count_channels(self):
        ...
        for count_info in result:
            try:
                ...
                logger.info(f"Verifying channel {channel.name} with last message ID {last_message_id}")
                after_message = None
                if last_message_id:
                    try:
                        after_message = await channel.fetch_message(int(last_message_id))
                        logger.info(f"Found after message: {after_message.content} for channel {channel.name}")
                    except Exception:
                        after_message = None
                # Get all messages after last_message_id (if any)
                messages = []
                if after_message:
                    async for msg in channel.history(after=after_message, oldest_first=True):
                        messages.append(msg)
                        logger.info(f"Found message: {msg.content} for channel {channel.name}")
                else:
                    # If no last_message_id, just check the latest message
                    async for msg in channel.history(limit=1, oldest_first=False):
                        messages.append(msg)
                        logger.info(f"No last message ID found, found message: {msg.content} for channel {channel.name}")
                # Get or initialize count state
                current_count = count_info.get('current_count', 0)
                highest_count = count_info.get('highest_count', 0)
                last_user = count_info.get('last_user')
                for message in messages:
                    ...
                    def safe_eval(expr):
                        ...
                        try:
                            ...
                            return None
                    number = safe_eval(expr)
                    logger.info(f"Evaluated expression {expr} to {number} for channel {channel.name}")
                    if number is None:
                        logger.info(f"Expression {expr} is not a valid number for channel {channel.name}")
                        continue
                    if number != current_count + 1:
                        ...
                        continue
                    if last_user == str(message.author.id):
                        logger.info(f"Last user {last_user} is the same as the current user {message.author.id} for channel {channel.name}")
                        try:
                            await message.add_reaction('❌')
                        except Exception:
                            pass
